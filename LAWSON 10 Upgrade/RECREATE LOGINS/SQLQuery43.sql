USE [master] 
IF NOT EXISTS (SELECT * FROM sys.server_principals WHERE name = N'SteveDBA')
BEGIN
DECLARE @CreateError INT
CREATE LOGIN [SteveDBA] WITH PASSWORD = 0x010039F4814717902E7A2ECD756D0DF193919E6E1123D30DC240 HASHED, SID = 0x248B6A195E1BB645BA521FD66DE60DB5, DEFAULT_LANGUAGE = us_english, CHECK_POLICY = OFF, CHECK_EXPIRATION = OFF
SET @CreateError = @@ERROR


GRANT CONNECT SQL TO [SteveDBA] 


USE [msdb]
IF NOT EXISTS (SELECT * FROM sys.database_principals WHERE name = N'SteveDBA')
CREATE USER [SteveDBA] FOR LOGIN [SteveDBA] WITH DEFAULT_SCHEMA=[dbo]
...
EXEC sp_addrolemember N'SQLAgentUserRole', N'SteveDBA'
...
EXEC sp_addrolemember N'SQLAgentReaderRole', N'SteveDBA'
...
EXEC sp_addrolemember N'SQLAgentOperatorRole', N'SteveDBA'

IF EXISTS (SELECT * FROM sys.database_principals WHERE name = N'db_owner' AND type = 'R')
EXEC sp_addrolemember N'db_owner', N'SteveDBA'



ALTER LOGIN [SteveDBA] ENABLE
