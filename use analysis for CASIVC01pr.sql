...
,MIN(datediff(MINUTE,logintime,CURRENT_TIMESTAMP)) as 'Minutes since last connection'
...
WHERE dbname not in ('DBA','master','msdb','tempdb','model','AAGtestdb','mmdata') -- DON'T CARE ABOUT THESE
--WHERE dbname = 'PlayerTracking'
  and loginname not in ('NT AUTHORITY\SYSTEM','NT SERVICE\SQLSERVERAGENT','NT SERVICE\MSSQLSERVER','initialize','VIEJAS\iderasvc01') -- MAINTENANCE JOBS DON'T COUNT AS BEING IN USE
