...
/****** Object:  Job [DBA - Alert on Low Disk Space v2]    Script Date: 5/16/2024 3:39:03 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Viejas DB Maintenance]    Script Date: 5/16/2024 3:39:03 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA - Alert on Low Disk Space v2', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'This job will send a text message and Email when any drive on the server is 10% or less free.  It will send out just an Email when any drive is 20% or less free.', 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Check Space and Send Alerts]    Script Date: 5/16/2024 3:39:03 PM ******/
...
,CASE 
   WHEN Capacity_MB >= 1000000 THEN 200
   WHEN Capacity_MB <= 200000 THEN 20 
   ELSE (Capacity_MB / 10000)
 END 
...
DECLARE @msg_SubjectLine VARCHAR(255) ;
...
BEGIN
	...
	AND (SELECT MAX(TV_Prod_or_QA) FROM @TV_Disk_Space_Alerts_Control) = ''P'' -- IS PROD CONDITION
	...
	-- Send Email alert for space under 20% free - to Apps team olnly and regardless of if the above alert was triggered

	IF (SELECT TV_Pct_Free FROM @TV_Drive_Stats WHERE TV_Number = @N) < 20

	BEGIN
		SET @msg_SubjectLine = @@SERVERNAME + '' '' + @msg_DriveLetter +''-Drive Is Under 20% Free -- Freespace is '' + @msg_Pct_Free + ''%'' ;

		EXEC msdb.dbo.sp_send_dbmail
		  @profile_name = ''Alerts''
		--, @recipients = ''spaylor@viejas.com;''
		, @recipients = ''itdba@viejas.com;''
		, @subject = @msg_SubjectLine
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Every 30 minutes', 
		...
		@schedule_uid=N'809a0a13-7a0c-47e5-955f-11116b0c4c1d'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
