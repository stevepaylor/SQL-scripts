...
INSERT INTO #current_casino_hosts 
	...
	VALUES
		...
		(21, 10000377, 'Luangviseth', 'Samantha', 'Luangviseth, Samantha', @start_date);

/*
Host Order number where hosts for termed hosts
*/

DECLARE

	@termed_cutoff int = 21;
...
WHERE
	...
	CONVERT(date, gaming_date) BETWEEN @start_date AND @end_date
...
-- manual adjustments to hosts who started after 1/1/2019

UPDATE #current_casino_hosts

SET

	days_passed = CASE WHEN host_last_name = ''
				THEN DATEDIFF(DAY, host_start_date, (SELECT MAX(gaming_date) FROM #all_player_ratings)) + 1 -- +
				WHEN host_last_name = ''
				THEN DATEDIFF(DAY, host_start_date, (SELECT MAX(gaming_date) FROM #all_player_ratings)) + 1 -- +
				ELSE DATEDIFF(DAY, host_start_date, (SELECT MAX(gaming_date) FROM #all_player_ratings)) + 1
				END;
...
INNER JOIN viewPlayersViejas AS p
...
WHERE
	...
	GamingDt BETWEEN @start_date AND @end_date
	AND IsVoid = 0
...
WHERE
	...
	AND acd.gaming_date BETWEEN acd.host_start_date AND @end_date;
...
SELECT
	...
	hg.guest_first_name,
	hg.club_status,
	COALESCE(ar.actual_win_total, 0) AS actual_win_total,
	COALESCE(ar.actual_win_slots, 0) AS actual_win_slots,
	COALESCE(ar.actual_win_table, 0) AS actual_win_table,
	COALESCE(ar.actual_win_bingo, 0) AS actual_win_bingo,
	COALESCE(ar.theo_win_total, 0) AS theo_win_total,
	COALESCE(ar.theo_win_slots, 0) AS theo_win_slots,
	COALESCE(ar.theo_win_table, 0) AS theo_win_table,
	COALESCE(ar.theo_win_bingo, 0) AS theo_win_bingo,
	COALESCE(ar.total_fpc_awarded, 0) AS total_fpc_awarded,
	COALESCE(ar.fpc_vouchers, 0) AS fpc_vouchers,
	COALESCE(ar.total_fpc_comps, 0) AS total_fpc_comps,
	COALESCE(ar.total_hotel_comps, 0) AS total_hotel_comps,
	COALESCE(ar.total_fb_comps, 0) AS total_fb_comps,
	COALESCE(ar.retail_comps, 0) AS retail_comps,
	COALESCE(ar.total_other_comps, 0) AS total_other_comps,
	COALESCE(ar.food_vouchers, 0) AS food_vouchers,
	COALESCE(ar.retail_vouchers, 0) AS retail_vouchers,
	COALESCE(ar.other_vouchers, 0) AS other_vouchers,
	COALESCE(ar.total_awards, 0) AS total_awards,
	COALESCE(ar.theo_trips, 0) AS trips

INTO #all_hosted_ratings_awards

FROM #hosted_guests AS hg

LEFT OUTER JOIN aggregated_ratings AS ar
ON hg.player_id = ar.player_id AND hg.player_account = ar.player_account

ORDER BY

	hg.host_last_name ASC,
	COALESCE(ar.theo_win_total, 0) DESC;

/*
result set for WTD Data
*/
...
WHERE
	...
	gaming_date BETWEEN DATEADD(DAY, -6, @end_date) AND @end_date
	AND (host_emp_id NOT IN (SELECT host_emp_id_perm FROM #current_casino_hosts WHERE host_order_number >= @termed_cutoff)
	AND host_emp_id NOT IN (SELECT host_emp_id_lead FROM #current_casino_hosts WHERE host_order_number >= @termed_cutoff))
...
SELECT
	...
	SUM(total_other_comps) AS total_other_comps,
	SUM(other_vouchers) AS other_vouchers,
	COUNT(player_id) AS wtd_head_count
...
/*
Produces Result Set for Club Summary 1/2
*/
...
SELECT
	...
	apr.player_id,
	SUM(theo_win_slots) + SUM(theo_win_table) AS total_theo

FROM #all_player_ratings AS apr

INNER JOiN viewPlayersViejas AS p
ON apr.player_id = p.PlayerId
...
FROM all_guests_ratings AS agr

INNER JOIN viewPlayersViejas AS p
ON agr.player_id = p.PlayerId
...
SELECT
	...
	club_status,
	COUNT(player_id) AS total_head_count,
	SUM(total_theo) AS total_theo_win


FROM all_guests_club_w_ratings
...
ORDER BY
	...
	CASE WHEN club_status = 'PLATINUM'
	THEN 1
	WHEN club_status = 'GOLD'
	THEN 2
	WHEN club_status = 'SILVER'
	THEN 3
	ELSE 4
	END ASC;

/*
Produces Result Set for Club Summary 2/2
*/
...
SELECT
	...
	ahra.player_id,
	ahra.club_status,
	ahra.theo_win_slots + ahra.theo_win_table AS theo_win_total

FROM #all_hosted_ratings_awards AS ahra

INNER JOIN #current_casino_hosts AS cch
ON (ahra.host_emp_id = cch.host_emp_id_perm OR ahra.host_emp_id = cch.host_emp_id_lead)
...
SELECT
	...
	club_status,
	COUNT(player_id) AS total_head_count,
	SUM(theo_win_total) AS hosted_theo_win

FROM hosted_club_summary
...
/*
		...
		-- Produces result set for individual host tabs
		-- update start dates in where clause manually for new hosts
		-- watchlist cutoff is manually updated
...
CREATE PROCEDURE #host_report_card
...
SELECT
	...
	CASE WHEN
		(
		CASE WHEN ahra.theo_win_total = 0
		THEN 0.0
		ELSE ahra.total_awards/ahra.theo_win_total * 1.0
		END
		) >= 0.25 -- watchlist cutoff
	THEN
	CONCAT
	(
	ahra.host_full_name,
		...
		nvarchar(50),
			ROW_NUMBER() 
			OVER 
			(PARTITION BY ahra.host_full_name 
			ORDER BY
				CASE WHEN ahra.theo_win_total = 0
			THEN 0.0
			ELSE ahra.total_awards/ahra.theo_win_total * 1.0
			END DESC,
			ahra.theo_win_total DESC
			)
		)
	)
	END AS watch_list,
	...
	ahra.player_account,
	ahra.guest_last_name,
	ahra.guest_first_name,
	ahra.club_status,
	ahra.trips,
	ahra.actual_win_total,
	ahra.actual_win_slots,
	ahra.actual_win_table,
	ahra.actual_win_bingo,
	ahra.theo_win_total,
	ahra.theo_win_slots,
	ahra.theo_win_table,
	ahra.theo_win_bingo,
	ahra.total_fpc_awarded,
	ahra.fpc_vouchers,
	ahra.total_fpc_comps,
	CONVERT(money, ahra.total_hotel_comps) AS total_hotel_comps,
	ahra.total_fb_comps,
	ahra.food_vouchers,
	ahra.retail_comps,
	ahra.retail_vouchers,
	ahra.total_other_comps,
	ahra.other_vouchers,
	CASE WHEN ahra.theo_win_total = 0
	THEN 0.0
	ELSE ahra.total_awards/ahra.theo_win_total * 1.0
	END AS ReinvestmentRate,
	CONVERT(money, ahra.total_awards) AS total_awards,
	CONVERT(money, (ahra.total_fb_comps + ahra.total_hotel_comps + ahra.total_other_comps + ahra.total_fpc_comps + ahra.retail_comps)) AS comp_awards,
	CASE WHEN ahra.total_awards = 0
	THEN 0.0
	ELSE (ahra.total_fb_comps + ahra.total_hotel_comps + ahra.total_other_comps + ahra.total_fpc_comps + ahra.retail_comps) / ahra.total_awards * 1.0
	END AS comp_award_perceentage
...
WHERE
	...
	cch.host_order_number = @host_order;
...
DECLARE
	...
	@CTR int = 
				...
				(
				SELECT
				MAX(host_order_number)
				FROM #current_casino_hosts
				)
...
EXECUTE #host_report_card @host_order = @INT
