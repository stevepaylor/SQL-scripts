...
where bpdP.start_date_time >='03/01/2022'
...
select tenderedStartDate, * from #Results t
where tenderedStartDate>='2022-04-11'  and (slot_theo_win + table_theo_win + other_theo_win)=0
order by acct, t.tenderedStartDate asc

---and len(acct)<6
order by acct asc

select top 10000  acct, player_first_name, player_last_name, player_club_level,  check_number as CheckNumber, TenderedStartDate, servername, cashiername, gaming_days from  #Results 
where ACCT in
