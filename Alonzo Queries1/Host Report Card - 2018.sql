...
INSERT INTO #current_casino_hosts 
	...
	VALUES
		...
		(1, 10000574, 'Angulo & Ng', 'Antonio', 'Angulo & Ng', @start_date),
		(1, 10000585, 'Angulo & Ng', 'Jennifer', 'Angulo & Ng', @start_date),
		(2, 10000314, 'Arcamo', 'Cecilie', 'Arcamo, Cecilie', @start_date),
		(3, 10000168, 'Eisner', 'Kimberly', 'Eisner, Kimberly', @start_date),
		(4, 10000262, 'Gil', 'Ivonne', 'Gil, Ivonne', @start_date),
		(5, 10000242, 'Gonzales', 'Jenn', 'Gonzales, Jenn', @start_date),
		(6, 502, 'Guzzetta', 'Carmela', 'Guzzetta, Carmela', @start_date),
		(7, 126, 'Johnston', 'Katy', 'Johnston, Katy', @start_date),
		(8, 385, 'Jupin', 'Rae', 'Jupin, Rae', @start_date),
		(9, 10000254, 'Lopez', 'Alfonso', 'Lopez, Alfonso', @start_date),
		(10, 10000377, 'Luangviseth', 'Samantha', 'Luangviseth, Samantha', @start_date),
		(11, 1427, 'Rogers', 'Chloe', 'Rogers, Chloe', @start_date),
		(12, 10000156, 'Shorees', 'Waleed', 'Shorees, Waleed', @start_date),
		(13, 1444, 'Soto', 'Luz', 'Soto, Luz', @start_date),
		(14, 1606, 'Stautz', 'Risa', 'Stautz, Risa', @start_date),
		(15, 861, 'Willan', 'Nathaniel', 'Willan, Nathaniel', @start_date),
		(16, 459, 'Wong', 'Tina', 'Wong, Tina', @start_date),

		--------------------------------------------------------------------------------------
				-- termed hosts
		--------------------------------------------------------------------------------------

		(17, 99, 'Bengel', 'Tyler', 'Tyler Bengel', @start_date),
		(18, 10000308, 'Cobian', 'Gabriel', 'Gabriel Cobian', @start_date),
		(19, 10000219, 'Dormishian', 'Shawn', 'Shawn Dormishian', @start_date),
		(20, 10000240, 'Orozco', 'Gilbert', 'Orozco, Gilbert', @start_date);
...
select * from #current_casino_hosts 
DECLARE

	@termed_cutoff int = 17;
...
WHERE
	...
	CONCAT(host_last_name,'-Lead') = e.LastName
	AND host_first_name = e.FirstName
	AND host_last_name <> 'Angulo & Ng';

/*
Adds lead empi_ds to #current_casino_hosts
for all combined hosts: Angulo & Ng
*/
...
WHERE
	...
	host_last_name = 'Angulo & Ng'
	AND 'Angulo-Lead' = e.LastName
	AND host_first_name = e.FirstName;
...
select * from #fpc_comp_prize_codes
...
select * from #all_player_ratings
...
select * from #awards_by_prize_code
...
select * from #hosted_guest_award_matrix
...
select * from #total_daily_fpc_comps
...
select * from  #hosted_daily_fpc_comps
...
select * from #total_non_fpc_comps_used
...
LEFT OUTER JOIN Love_Test.dbo.host_report_card_ratings_hist AS hrcr
