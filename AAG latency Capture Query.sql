;WITH AG_Stats AS (
    ...
    SDSDB13CPR_CommitTime AS 
            (
            SELECT  DBName, last_commit_time
            FROM    AG_Stats
            WHERE   role_desc = 'SECONDARY' AND [InstanceName] = 'SDSDB13CPR'
            ),
    FO_CommitTime AS 
            (
            SELECT  DBName, last_commit_time
            FROM    AG_Stats
            WHERE   role_desc = 'SECONDARY' AND ([InstanceName] = 'SDSDB13CPR' OR [InstanceName] = 'SDSDB13CPR')
            )
INSERT INTO DBA.dbo.NodeC_Latency_Tracking
SELECT p.[DBName] AS [DatabaseName], p.last_commit_time AS [Primary_Last_Commit_Time]
    , r.last_commit_time AS [SDSDB13CPR_Last_Commit_Time]
    , DATEDIFF(ss,r.last_commit_time,p.last_commit_time) AS [SDSDB13CPR_Sync_Lag_(secs)]
...
--INTO DBA.dbo.NodeC_Latency_Tracking

FROM Pri_CommitTime p
LEFT JOIN SDSDB13CPR_CommitTime r ON [r].[DBName] = [p].[DBName]
LEFT JOIN FO_CommitTime f ON [f].[DBName] = [p].[DBName]

WHERE DATEDIFF(ss,r.last_commit_time,p.last_commit_time) > 5
   OR DATEDIFF(ss,f.last_commit_time,p.last_commit_time) > 5