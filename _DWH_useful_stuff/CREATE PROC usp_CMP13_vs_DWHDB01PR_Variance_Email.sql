...
DROP PROCEDURE IF EXISTS dbo.usp_CMP13_vs_DWHDB01PR_Variance_Email
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE dbo.usp_CMP13_vs_DWHDB01PR_Variance_Email
AS
--=========================================================================================================
-- Author: SPaylor
-- Created:  08/01/2023
...
-- on CMKTDBDPR and its corresponding table in the iQ-Gaming database on DWHDB01PR. It is include in the 
...
-------------------------------------------------------------------------------------------------------------
TRUNCATE TABLE SystemMaintDB.dbo.CMP13_DWHDB01PR_metrics ;

----DROP TABLE IF EXISTS SystemMaintDB.dbo.CMP13_DWHDB01PR_metrics_temp ;

INSERT INTO SystemMaintDB.dbo.CMP13_DWHDB01PR_metrics 
...
, VarCount = 
  CASE
	...
	ELSE MAX(CAST(DWHDB01PR as int)) - MAX(CAST(CMKTDB13DPR as int))
  END
...
, VarPct = 
  CASE
	WHEN SortKey NOT IN(1,6,7) THEN NULL
	WHEN MAX(CAST(CMKTDB13DPR as int)) = 0 THEN NULL
	ELSE FORMAT(MAX(CAST(CMKTDB13DPR as int)) * 1.0 / MAX(CAST(DWHDB01PR as int)) -1 , 'N6')
  END
...
-------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS SystemMaintDB.dbo.CMP13_DWHDB01PR_metrics_For_Email ;
...
, DWHDB01PR =
  CASE 
	WHEN SortKey IN (2,3,4,5)
	THEN DWHDB01PR
	ELSE FORMAT(CAST(DWHDB01PR as int),'N0')
  END

, ISNULL(FORMAT(VarCount , 'N0'), ' ') VarCount
, ISNULL(FORMAT(CAST(VarPct as numeric(7,4)) , 'N4'), ' ') VarPct

INTO SystemMaintDB.dbo.CMP13_DWHDB01PR_metrics_For_Email

FROM SystemMaintDB.dbo.CMP13_DWHDB01PR_metrics


WHERE ((CMKTDB13DPR IS NOT NULL AND CMKTDB13DPR <> ' ') -- suppress displaying rows with no metrics
   OR (DWHDB01PR IS NOT NULL AND DWHDB01PR <> ' '))
...
EXEC DBA.dbo.SpCustomTable2HTML 
     'SystemMaintDB.dbo.CMP13_DWHDB01PR_metrics_For_Email', 
...
DECLARE @subject_value as VARCHAR(300) = 'Table Sync Validation for CMP13 to DWHDB01PR ' + CONVERT(VARCHAR(10),DATEADD(D,-1,CURRENT_TIMESTAMP),101) + '  --  ' + @@SERVERNAME;
