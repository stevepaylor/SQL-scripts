...
CREATE   PROCEDURE [myViejas].[sp_IQ_Marketing_Metrics_Update_EXT_Steve]
		@baseLineDate DATE,
        @beginningDate DATE,
		@CorporateRecordsExist CHAR(1) = 'N',
		@UseAccountingDayData CHAR(1) = 'Y',
		@CustomMMEXTGroupingProcedure varchar(70) = NULL,
		@SourceDatabase varchar(30) = NULL
AS

BEGIN

	--drop global tamp tables if they exist
	IF OBJECT_ID('tempdb..##RawRatings_MM') IS NOT NULL    
		drop table ##RawRatings_MM

	--drop local tamp tables if they exist
	IF OBJECT_ID('tempdb..#RawMarketingMetricsEXT_MM') IS NOT NULL    
		drop table #RawMarketingMetricsEXT_MM
	IF OBJECT_ID('tempdb..#SummedMarketingMetricsEXT_MM') IS NOT NULL    
		drop table #SummedMarketingMetricsEXT_MM
	IF OBJECT_ID('tempdb..#SummedMarketingMetricsCORPEXT_MM') IS NOT NULL    
		drop table #SummedMarketingMetricsCORPEXT_MM
	IF OBJECT_ID('tempdb..#MarketingMetricsEXT_MM') IS NOT NULL    
		drop table #MarketingMetricsEXT_MM
	IF OBJECT_ID('tempdb..#CustomRawExtract_MM') IS NOT NULL
		drop table #CustomRawExtract_MM
    IF OBJECT_ID('tempdb..#UniqueDays_MM') IS NOT NULL
		drop table #UniqueDays_MM
	IF OBJECT_ID('tempdb..#SummedCustomExtract_MM') IS NOT NULL
		drop table #SummedCustomExtract_MM
	IF OBJECT_ID('tempdb..#GamingPreferences_MM') IS NOT NULL
		drop table #GamingPreferences_MM
	IF OBJECT_ID('tempdb..#RatingMinutes_MM') IS NOT NULL
		drop table #RatingMinutes_MM
	IF OBJECT_ID('tempdb..#GamingPreferencesBase_MM') IS NOT NULL
		drop table #GamingPreferencesBase_MM
	IF OBJECT_ID('tempdb..#GamingPreferencesCustom_MM') IS NOT NULL
		drop table #GamingPreferencesCustom_MM
	IF OBJECT_ID('tempdb..#GamingPreferencesCORP_MM') IS NOT NULL
		drop table #GamingPreferencesCORP_MM
	IF OBJECT_ID('tempdb..#FinalPreferences_MM') IS NOT NULL
		drop table #FinalPreferences_MM
	IF OBJECT_ID('tempdb..#MMGameTypePref_MM') IS NOT NULL
		drop table #MMGameTypePref_MM
	IF OBJECT_ID('tempdb..#PropertyPreference_MM') IS NOT NULL
		drop table #PropertyPreference_MM
	IF OBJECT_ID('tempdb..#GuestStatus_MM') IS NOT NULL
		drop table #GuestStatus_MM
	IF OBJECT_ID('tempdb..#Distances_MM') IS NOT NULL
		drop table #Distances_MM
	IF OBJECT_ID('tempdb..#Decliners_MM') IS NOT NULL
		drop table #Decliners_MM
	IF OBJECT_ID('tempdb..#Awards_MM') IS NOT NULL
		drop table #Awards_MM
	IF OBJECT_ID('tempdb..#RatingMinutesGameCode_MM') IS NOT NULL
		drop table #RatingMinutesGameCode_MM
	IF OBJECT_ID('tempdb..#RatingMinutesGameType_MM') IS NOT NULL
		drop table #RatingMinutesGameType_MM
	...
	SELECT
		cast(PlayerID as varchar(12)) + cast(CasinoCode as char(4)) as UniqueKey, 
		playerID,
		CasinoCode,
		cast(GamingDate as date) as [Date],
		GameType,
		GameCode,
		MinutesPlayed as Rating_Minutes			--PORT
	INTO ##RawRatings_MM
	FROM [dbo].[IQC_Player_Ratings]
	--WHERE cast(GamingDate as date) >  @beginningDate
	WHERE GamingDate > @beginningDate

	--This pulls in the PlayerType variables
	DROP TABLE IF EXISTS [##playerType_MM] 

	SELECT p.playerID
	,'VCR' as CasinoCode
	,cast(p.PlayerID as varchar(12)) + cast('VCR' as char(4)) as UniqueKey
	,p.PlayerTypeID
	,pt.[PlayerTypeCode]
	,pt.[PlayerTypeName]
	INTO ##playerType_MM
	FROM [iQ-Gaming].[CMP_13-1].tPlayer p
	LEFT JOIN [iQ-Gaming].[CMP_13-1].tPlayerType pt on p.playerTypeID=pt.playerTypeID
	   	 
	-- Create the Raw Marketing Metrics data. These are Additional variables that supplement time silo defined 
	-- in the base program added LastTrip date so can calculate the 3MonthLGD numbers.
	-- 3MonthLGD is the the 3 months ratings from the last gamingday.  If last gaming day is 7/5/2018 the LGD 
	-- timeframe is 7/31/2019 back through 5/1/2019, 3 months total

	SELECT	cast(a.PlayerID as varchar(12)) + cast(a.CasinoCode as char(4)) as UniqueKey, 
		   	[Date],
			b.LastTripDate,
			eomonth(b.LastTripDate) as max_3MonthLGD,
			DATEADD(day, 1, EOMONTH(DATEADD(month,-3,eomonth(b.LastTripDate)))) as min_3MonthLGD,
	...
	INTO #RawMarketingMetricsEXT_MM
	FROM ##DailyTotalsExtract_MM a
	LEFT JOIN
	(
		select PlayerID, CasinoCode, LastTripDate from ##LGD_Extract_MM --##LastTrips
	) b
	ON a.PlayerID = b.PlayerID	and a.CasinoCode = b.CasinoCode
	WHERE a.playerID <> 0

	-- Create an index to improve processing
	CREATE CLUSTERED INDEX IDX_Player_Casino_EXT ON #RawMarketingMetricsEXT_MM(UniqueKey)

	-- Summarize detail data by PlayerID, CasinoCode

	SELECT UniqueKey,
		sum(daysOnProperty_last36Months) as daysOnProperty_last36Months,
		sum(daysOnProperty_last4Months) as daysOnProperty_last4Months,
		sum(daysOnProperty_last2Months) as daysOnProperty_last2Months,
		sum(daysOnProperty_3MonthLGD) as daysOnProperty_3MonthLGD,
		sum(gamingDays_last36Months) as gamingDays_last36Months,
		sum(gamingDays_last4Months) as gamingDays_last4Months,
		sum(gamingDays_last2Months) as gamingDays_last2Months,
		sum(gamingDays_3MonthLGD) as gamingDays_3MonthLGD,
		sum(slotDays_last36Months) as slotDays_last36Months,
		sum(slotDays_last4Months) as slotDays_last4Months,
		sum(slotDays_last2Months) as slotDays_last2Months,
		sum(slotDays_3MonthLGD) as slotDays_3MonthLGD,
		sum(tableDays_last36Months) as tableDays_last36Months,
		sum(tableDays_last4Months) as tableDays_last4Months,
		sum(tableDays_last2Months) as tableDays_last2Months,
		sum(tableDays_3MonthLGD) as tableDays_3MonthLGD,
		count(distinct TripNumber_Last36Months) as trips_last36Months,
		count(distinct TripNumber_Last4Months) as trips_last4Months,
		count(distinct TripNumber_Last2Months) as trips_last2Months,
		count(distinct TripNumber_3MonthLGD) as trips_3MonthLGD,
		sum(totalTheo_Last36Months) as totalTheo_last36Months,
		sum(totalTheo_Last4Months) as totalTheo_last4Months,
		sum(totalTheo_Last2Months) as totalTheo_last2Months,
		sum(totalTheo_3MonthLGD) as totalTheo_3MonthLGD,
		sum(actualWinLoss_Last36Months) as actualWinLoss_last36Months,
		sum(actualWinLoss_Last4Months) as actualWinLoss_last4Months,
		sum(actualWinLoss_Last2Months) as actualWinLoss_last2Months,
		sum(actualWinLoss_3MonthLGD) as actualWinLoss_3MonthLGD,
		sum(slotTheo_Last36Months) as slotTheo_last36Months,
		sum(slotTheo_Last4Months) as slotTheo_last4Months,
		sum(slotTheo_Last2Months) as slotTheo_last2Months,
		sum(slotTheo_3MonthLGD) as slotTheo_3MonthLGD,
		sum(tableTheo_Last36Months) as tableTheo_last36Months,
		sum(tableTheo_Last4Months) as tableTheo_last4Months,
		sum(tableTheo_Last2Months) as tableTheo_last2Months,
		sum(tableTheo_3MonthLGD) as tableTheo_3MonthLGD,
		sum(slotActualWinLoss_Last36Months) as slotActualWinLoss_last36Months,
		sum(slotActualWinLoss_Last4Months) as slotActualWinLoss_last4Months,
		sum(slotActualWinLoss_Last2Months) as slotActualWinLoss_last2Months,
		sum(slotActualWinLoss_3MonthLGD) as slotActualWinLoss_3MonthLGD,
		sum(tableActualWinLoss_Last36Months) as tableActualWinLoss_last36Months,
		sum(tableActualWinLoss_Last4Months) as tableActualWinLoss_last4Months,
		sum(tableActualWinLoss_Last2Months) as tableActualWinLoss_last2Months,
		sum(tableActualWinLoss_3MonthLGD) as tableActualWinLoss_3MonthLGD,
		sum(compsEarned_Last36Months) as compsEarned_last36Months,
		sum(compsEarned_Last4Months) as compsEarned_last4Months,
		sum(compsEarned_Last2Months) as compsEarned_last2Months,
		sum(compsEarned_3MonthLGD) as compsEarned_3MonthLGD,
		sum(compsUsed_Last36Months) as compsUsed_last36Months,
		sum(compsUsed_Last4Months) as compsUsed_last4Months,
		sum(compsUsed_Last2Months) as compsUsed_last2Months,
		sum(compsUsed_3MonthLGD) as compsUsed_3MonthLGD,
		sum(pointsEarned_Last36Months) as pointsEarned_last36Months,
		sum(pointsEarned_Last4Months) as pointsEarned_last4Months,
		sum(pointsEarned_Last2Months) as pointsEarned_last2Months,
		sum(pointsEarned_3MonthLGD) as pointsEarned_3MonthLGD,
		sum(pointPlayUsed_Last36Months) as pointPlayUsed_last36Months,
		sum(pointPlayUsed_Last4Months) as pointPlayUsed_last4Months,
		sum(pointPlayUsed_Last2Months) as pointPlayUsed_last2Months,
		sum(pointPlayUsed_3MonthLGD) as pointPlayUsed_3MonthLGD,
		sum(dailyWorth_YTD ) as dailyWorth_YTD,
		sum(dailyWorth_QTD ) as dailyWorth_QTD,
		sum(dailyWorth_Last36Months ) as dailyWorth_Last36Months,
		sum(dailyWorth_last24Months ) as dailyWorth_last24Months,
		sum(dailyWorth_last18Months ) as dailyWorth_last18Months,
		sum(dailyWorth_last12Months ) as dailyWorth_last12Months,
		sum(dailyWorth_last9Months ) as dailyWorth_last9Months,
		sum(dailyWorth_last6Months ) as dailyWorth_last6Months,
		sum(dailyWorth_Last4Months ) as dailyWorth_Last4Months,
		sum(dailyWorth_last3Months ) as dailyWorth_last3Months,
		sum(dailyWorth_Last2Months ) as dailyWorth_Last2Months,
		sum(dailyWorth_lastMonth ) as dailyWorth_lastMonth,
		sum(dailyWorth_3MonthLGD ) as dailyWorth_3MonthLGD ,
		sum(BingoTheo_YTD ) as BingoTheo_YTD,
		sum(BingoTheo_QTD ) as BingoTheo_QTD,
		sum(BingoTheo_Last36Months ) as BingoTheo_Last36Months,
		sum(BingoTheo_last24Months ) as BingoTheo_last24Months,
		sum(BingoTheo_last18Months ) as BingoTheo_last18Months,
		sum(BingoTheo_last12Months ) as BingoTheo_last12Months,
		sum(BingoTheo_last9Months ) as BingoTheo_last9Months,
		sum(BingoTheo_last6Months ) as BingoTheo_last6Months,
		sum(BingoTheo_Last4Months ) as BingoTheo_Last4Months,
		sum(BingoTheo_last3Months ) as BingoTheo_last3Months,
		sum(BingoTheo_Last2Months ) as BingoTheo_Last2Months,
		sum(BingoTheo_lastMonth ) as BingoTheo_lastMonth,
		sum(BingoTheo_3MonthLGD ) as BingoTheo_3MonthLGD ,
		max(LastTripDate) as LastTripDate,
		max(max_3MonthLGD) as max_3MonthLGD,
		max(min_3MonthLGD) as min_3MonthLGD
		,count (distinct case when [Date] between DATEADD(year, DATEDIFF(year, 0 ,@baselineDate),0) and @baselineDate 
			then format(Date,'yyyyMM') 	else null end) 
		as MonthsPlayed_YTD
		,count (distinct case when [Date] between DATEADD(quarter, DATEDIFF(quarter, 0 ,@baselineDate),0) and @baselineDate
				then format(Date,'yyyyMM') 	else null end) 
		as MonthsPlayed_QTD
		,count (distinct case when [Date] between DATEADD(day, 1, EOMONTH(DATEADD(month,-36,@baselineDate))) and @baselineDate
				then format(Date,'yyyyMM') 	else null end) 
		as MonthsPlayed_Last36Months
		,count (distinct case when [Date] between DATEADD(day, 1, EOMONTH(DATEADD(month,-24,@baselineDate))) and @baselineDate
				then format(Date,'yyyyMM') 	else null end) 
		as MonthsPlayed_last24Months
		,count (distinct case when [Date] between DATEADD(day, 1, EOMONTH(DATEADD(month,-18,@baselineDate))) and @baselineDate
				then format(Date,'yyyyMM') 	else null end) 
		as MonthsPlayed_last18Months
		,count (distinct case when [Date] between DATEADD(day, 1, EOMONTH(DATEADD(month,-12,@baselineDate))) and @baselineDate
				then format(Date,'yyyyMM') 	else null end) 
		as MonthsPlayed_last12Months
		,count (distinct case when [Date] between DATEADD(day, 1, EOMONTH(DATEADD(month,-9,@baselineDate))) and @baselineDate
				then format(Date,'yyyyMM') 	else null end) 
		as MonthsPlayed_last9Months
		,count (distinct case when [Date] between DATEADD(day, 1, EOMONTH(DATEADD(month,-6,@baselineDate))) and @baselineDate
				then format(Date,'yyyyMM') 	else null end) 
		as MonthsPlayed_last6Months
		,count (distinct case when [Date] between DATEADD(day, 1, EOMONTH(DATEADD(month,-3,@baselineDate))) and @baselineDate
				then format(Date,'yyyyMM') 	else null end) 
		as MonthsPlayed_last3Months
		,count (distinct case when [Date] between DATEADD(day, 1, EOMONTH(DATEADD(month,-4,@baselineDate))) and @baselineDate 
				then format(Date,'yyyyMM') 	else null end) 
		as MonthsPlayed_Last4Months
		,count (distinct case when [Date] between DATEADD(day, 1, EOMONTH(DATEADD(month,-2,@baselineDate))) and @baselineDate
				then format(Date,'yyyyMM') 	else null end) 
		as MonthsPlayed_Last2Months
		,count (distinct case when [Date] between DATEADD(day, 1, EOMONTH(DATEADD(month,-1,@baselineDate))) and @baselineDate
				then format(Date,'yyyyMM') 	else null end)
		as MonthsPlayed_lastMonth
		,count (distinct case when [Date] between  DATEADD (day, 1, EOMONTH(DATEADD(month,-3,eomonth(LastTripDate)))) and eomonth(LastTripDate)
				then format(Date,'yyyyMM') 	else null end) 
		as MonthsPlayed_3MonthLGD,
		min(PlayDates_YTD ) as firstPlayDate_YTD,
		min(PlayDates_QTD ) as firstPlayDate_QTD,
		min(PlayDates_Last36Months ) as firstPlayDate_Last36Months,
		min(PlayDates_last24Months ) as firstPlayDate_last24Months,
		min(PlayDates_last18Months ) as firstPlayDate_last18Months,
		min(PlayDates_last12Months ) as firstPlayDate_last12Months,
		min(PlayDates_last9Months ) as firstPlayDate_last9Months,
		min(PlayDates_last6Months ) as firstPlayDate_last6Months,
		min(PlayDates_Last4Months ) as firstPlayDate_Last4Months,
		min(PlayDates_last3Months ) as firstPlayDate_last3Months,
		min(PlayDates_Last2Months ) as firstPlayDate_Last2Months,
		min(PlayDates_lastMonth ) as firstPlayDate_lastMonth,
		min(PlayDates_3MonthLGD ) as firstPlayDate_3MonthLGD,
		max(PlayDates_YTD ) as lastPlayDate_YTD,
		max(PlayDates_QTD ) as lastPlayDate_QTD,
		max(PlayDates_Last36Months ) as lastPlayDate_Last36Months,
		max(PlayDates_last24Months ) as lastPlayDate_last24Months,
		max(PlayDates_last18Months ) as lastPlayDate_last18Months,
		max(PlayDates_last12Months ) as lastPlayDate_last12Months,
		max(PlayDates_last9Months ) as lastPlayDate_last9Months,
		max(PlayDates_last6Months ) as lastPlayDate_last6Months,
		max(PlayDates_Last4Months ) as lastPlayDate_Last4Months,
		max(PlayDates_last3Months ) as lastPlayDate_last3Months,
		max(PlayDates_Last2Months ) as lastPlayDate_Last2Months,
		max(PlayDates_lastMonth ) as lastPlayDate_lastMonth,
		max(PlayDates_3MonthLGD ) as lastPlayDate_3MonthLGD
	INTO #SummedMarketingMetricsEXT_MM
	FROM #RawMarketingMetricsEXT_MM
	GROUP BY UniqueKey

	-- Do a similar pigeon-holing of the Awards table
	
	DROP TABLE IF EXISTS #AwardsExtract_MM 

	SELECT	cast(a.PlayerID as varchar(12)) + 'VCR' as UniqueKey, 
			a.GamingDate,
			b.LastTripDate,
			eomonth(b.LastTripDate) as max_3MonthLGD,
			...
			-------- Steve 10/23/2023 
			-------- Add missing computed columns: NetTheo_3MonthLGD, NetActualWin_3MonthLGD, SlotActualWin_3MonthLGD, TableNetActualWin_3MonthLGD, OtherNetActualWin_3MonthLGD
			...
			------case when [GamingDate] between DATEADD(month,-3,eomonth(b.LastTripDate)) and eomonth(b.LastTripDate) 
			------	then sum(pdtg.slot_theo_win + pdtg.table_theo_win + pdtg.other_theo_win) - (isnull(a.award_FPAwarded_Total,0) - isnull(a.award_FPAwarded_PromoWinner,0)) 
			------	else 0 
			------end 
			------as NetTheo_3MonthLGD,
			...
			------case when [GamingDate] between DATEADD(month,-3,eomonth(b.LastTripDate)) and eomonth(b.LastTripDate) 
			------	then sum(pdtg.slot_actual_win + pdtg.table_actual_win + pdtg.other_actual_win) - (isnull(a.award_FPAwarded_Total,0) - isnull(a.award_FPAwarded_PromoWinner,0)) 
			------	else 0 
			------end 				
			------as NetActualWin_3MonthLGD,

			--------Old DWH calc = (a.slot_actual_win - a.award_FPAwarded_Total)
			------case when [GamingDate] between DATEADD(month,-3,eomonth(b.LastTripDate)) and eomonth(b.LastTripDate) 
			------	then sum(pdtg.slot_actual_win) - (isnull(a.award_FPAwarded_Total,0)) 
			------	else 0 
			------end 				
			------as SlotActualWin_3MonthLGD,


			--------Old DWH calc = (a.table_actual_win - a.award_PromoChipsAwarded)
			------case when [GamingDate] between DATEADD(month,-3,eomonth(b.LastTripDate)) and eomonth(b.LastTripDate) 
			------	then sum(pdtg.table_actual_win) - (isnull(a.award_PromoChipsAwarded,0)) 
			------	else 0 
			------end 				
			------as TableNetActualWin_3MonthLGD,


			--------Old DWH calc = (a.other_actual_win - a.award_CashAwarded_Bingo)
			------case when [GamingDate] between DATEADD(month,-3,eomonth(b.LastTripDate)) and eomonth(b.LastTripDate) 
			------	then sum(pdtg.other_actual_win) - (isnull(a.award_CashAwarded_Bingo,0)) 
			------	else 0 
			------end 				
			------as OtherNetActualWin_3MonthLGD,
			...
			case when [GamingDate] between DATEADD(month,-3,eomonth(b.LastTripDate)) and eomonth(b.LastTripDate) then isnull(a.Award_FPAwarded_Total,0) else 0 end as Award_FPAwarded_Total_3MonthLGD,

-- steve and carl added on 
			case when [GamingDate] between DATEADD(month,-3,eomonth(b.LastTripDate)) and eomonth(b.LastTripDate) then isnull(a.Award_BingoCash_ActiveNL,0) else 0 end as Award_BingoCash_ActiveNL_3MonthLGD,

			case when [GamingDate] between DATEADD(month,-3,eomonth(b.LastTripDate)) and eomonth(b.LastTripDate) then isnull(a.Award_BingoCash_InactiveNL,0) else 0 end as Award_BingoCash_InactiveNL_3MonthLGD,

			case when [GamingDate] between DATEADD(month,-3,eomonth(b.LastTripDate)) and eomonth(b.LastTripDate) then isnull(a.Award_BingoCash_OuterMarketNL,0) else 0 end as Award_BingoCash_OuterMarketNL_3MonthLGD,

			case when [GamingDate] between DATEADD(month,-3,eomonth(b.LastTripDate)) and eomonth(b.LastTripDate) then isnull(a.Award_BingoCash_Supplemental,0) else 0 end as Award_BingoCash_Supplemental_3MonthLGD



INTO #AwardsExtract_MM				                                                                                                                                                                                                           
	FROM [iQ-Gaming].[dbo].[IQC_Awards] a
	LEFT JOIN
	(
		SELECT PlayerID, CasinoCode, LastTripDate from ##LGD_Extract_MM --##LastTrips
	) b
	...
	DROP TABLE IF EXISTS #AwardsSum_MM
	...
	---------------------------------------------------------------------------------------------------------------
		, SlotActualWinLoss_3MonthLGD - award_FPAwarded_Total_3MonthLGD as SlotActualWin_3MonthLGD
		, TableActualWinLoss_3MonthLGD -  award_PromoChipsUsed_Total_3MonthLGD as TableNetActualWin_3MonthLGD
		, BingoActualWinLoss_3MonthLGD - (Award_BingoCash_ActiveNL_3MonthLGD + Award_BingoCash_InactiveNL_3MonthLGD + Award_BingoCash_OuterMarketNL_3MonthLGD + Award_BingoCash_Supplemental_3MonthLGD) as OtherNetActualWin_3MonthLGD
		, (SlotTheo_3MonthLGD + TableTheo_3MonthLGD + BingoTheo_3MonthLGD) - (award_FPAwarded_Total_3MonthLGD - award_FPAwarded_PromoWinner_3MonthLGD) as NetTheo_3MonthLGD
		, (SlotActaulWinLoss_3MonthLGD + TableActaulWinLoss_3MonthLGD + BingoActaulWinLoss_3MonthLGD) - (award_FPAwarded_Total_3MonthLGD - award_FPAwarded_PromoWinner_3MonthLGD) as NetActualWin_3MonthLGD
	...
	RIGHT JOIN (select Uniquekey,
				...
				from #SummedMarketingMetricsEXT_MM) MME
	...
	DROP TABLE IF EXISTS #UniqueDays_MM

	SELECT coalesce (base.UniqueKey, rate.uniqueKey, awd.uniqueKey) as Uniquekey
		--ADP
		...
		,award_FPUsed_3MonthLGD
	INTO #UniqueDays_MM
	FROM [dbo].[IQ_Marketing_Metrics] base
	FULL JOIN #SummedMarketingMetricsEXT_MM rate ON base.UniqueKey=rate.UniqueKey
	LEFT JOIN #AwardsSum2_MM awd ON base.UniqueKey=awd.UniqueKey

	-- Create an index to improve processing
	CREATE CLUSTERED INDEX IDX_UniqueKey ON #UniqueDays_MM(UniqueKey)	
	...
	INTO #RatingMinutes_MM		  
	FROM ##RawRatings_MM a
	LEFT JOIN
		(select PlayerID, CasinoCode, LastTripDate from ##LGD_Extract_MM ) b
	ON a.PlayerID = b.PlayerID AND a.CasinoCode = b.CasinoCode
	WHERE Rating_Minutes > 0.0
	AND a.PlayerID <> 0

	--Aggregate #RatingMinutes_MM by GameCode and GameType

	--PORT GameCode

	SELECT PlayerID, CasinoCode, GameType, GameCode, 
		...
		sum(RatingMinutes_3MonthLGD) as RatingMinutes_3MonthLGD,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode, GameType ORDER BY SUM(RatingMinutes_last36Months) DESC) AS GameCodePref_Last36Months,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode, GameType ORDER BY SUM(RatingMinutes_last24Months) DESC) AS GameCodePref_Last24Months,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode, GameType ORDER BY SUM(RatingMinutes_last18Months) DESC) AS GameCodePref_Last18Months,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode, GameType ORDER BY SUM(RatingMinutes_last12Months) DESC) AS GameCodePref_Last12Months,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode, GameType ORDER BY SUM(RatingMinutes_last9Months) DESC) AS GameCodePref_Last9Months,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode, GameType ORDER BY SUM(RatingMinutes_last6Months) DESC) AS GameCodePref_Last6Months,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode, GameType ORDER BY SUM(RatingMinutes_last4Months) DESC) AS GameCodePref_Last4Months,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode, GameType ORDER BY SUM(RatingMinutes_last3Months) DESC) AS GameCodePref_Last3Months,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode, GameType ORDER BY SUM(RatingMinutes_last2Months) DESC) AS GameCodePref_Last2Months,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode, GameType ORDER BY SUM(RatingMinutes_lastMonth) DESC) AS GameCodePref_LastMonth,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode, GameType ORDER BY SUM(RatingMinutes_3MonthLGD) DESC) AS GameCodePref_3MonthLGD
	INTO #RatingMinutesGameCode_MM		  
	FROM #RatingMinutes_MM
	WHERE RatingMinutes_last36Months > 0
	GROUP BY PlayerID, CasinoCode, GameType, GameCode

	--PORT GameType

	SELECT PlayerID, CasinoCode, GameType,  
		sum(RatingMinutes_last36Months) as RatingMinutes_last36Months,			 
		sum(RatingMinutes_last24Months) as RatingMinutes_last24Months,			 
		sum(RatingMinutes_last18Months) as RatingMinutes_last18Months,			 
		sum(RatingMinutes_last12Months) as RatingMinutes_last12Months,			 
		...
		sum(RatingMinutes_lastMonth) as RatingMinutes_lastMonth,
		sum(RatingMinutes_3MonthLGD) as RatingMinutes_3MonthLGD,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode ORDER BY SUM(RatingMinutes_last36Months) DESC) AS GameTypePref_Last36Months,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode ORDER BY SUM(RatingMinutes_last24Months) DESC) AS GameTypePref_Last24Months,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode ORDER BY SUM(RatingMinutes_last18Months) DESC) AS GameTypePref_Last18Months,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode ORDER BY SUM(RatingMinutes_last12Months) DESC) AS GameTypePref_Last12Months,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode ORDER BY SUM(RatingMinutes_last9Months) DESC) AS GameTypePref_Last9Months,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode ORDER BY SUM(RatingMinutes_last6Months) DESC) AS GameTypePref_Last6Months,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode ORDER BY SUM(RatingMinutes_last4Months) DESC) AS GameTypePref_Last4Months,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode ORDER BY SUM(RatingMinutes_last3Months) DESC) AS GameTypePref_Last3Months,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode ORDER BY SUM(RatingMinutes_last2Months) DESC) AS GameTypePref_Last2Months,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode ORDER BY SUM(RatingMinutes_lastMonth) DESC) AS GameTypePref_LastMonth,
		ROW_NUMBER() OVER(PARTITION BY PlayerID, CasinoCode ORDER BY SUM(RatingMinutes_3MonthLGD) DESC) AS GameTypePref_3MonthLGD
	INTO #RatingMinutesGameType_MM		  
	FROM #RatingMinutes_MM
	WHERE RatingMinutes_last36Months > 0
	GROUP BY PlayerID, CasinoCode, GameType
    ...
    SELECT 
		...
		a.GameType,
		a.GameCode,
		b.GameTypePref_Last36Months,
		a.GameCodePref_Last36Months,
		b.GameTypePref_Last24Months,
		a.GameCodePref_Last24Months,
		b.GameTypePref_Last18Months,
		a.GameCodePref_Last18Months,
		b.GameTypePref_Last12Months,
		a.GameCodePref_Last12Months,
		b.GameTypePref_Last9Months,
		a.GameCodePref_Last9Months,
		b.GameTypePref_Last6Months,
		a.GameCodePref_Last6Months,
		b.GameTypePref_Last4Months,
		a.GameCodePref_Last4Months,
		b.GameTypePref_Last3Months,
		a.GameCodePref_Last3Months,
		b.GameTypePref_Last2Months,
		a.GameCodePref_Last2Months,
		b.GameTypePref_LastMonth,
		a.GameCodePref_LastMonth,
		b.GameTypePref_3MonthLGD,
		a.GameCodePref_3MonthLGD
	INTO #GamingPreferencesBase_MM
	FROM #RatingMinutesGameCode_MM a
	INNER JOIN #RatingMinutesGameType_MM b ON a.PlayerID = b.PlayerID AND a.CasinoCode = b.CasinoCode AND a.GameType = b.GameType
 
	 --PORT - we do not need to create CORP records 
	...
	INTO #GamingPreferencesCORP_DEBUG
	FROM #GamingPreferencesBase_DEBUG*/

	-- Create final preference file (without CORP)
	...
	INTO #FinalPreferences_MM         
    FROM (select cast(PlayerID as varchar(12)) + cast(CasinoCode as char(4)) as UniqueKey,
		case when GameTypePref_Last36Months = 1 then GameType else null end as GameTypePref_Last36Months,			--PORT
		case when GameTypePref_Last36Months = 1 and GameCodePref_Last36Months = 1 then GameCode else null end as GameCodePref_Last36Months,
		case when GameTypePref_Last24Months = 1 then GameType else null end as GameTypePref_Last24Months,
		case when GameTypePref_Last24Months = 1 and GameCodePref_Last24Months = 1 then GameCode else null end as GameCodePref_Last24Months,
		case when GameTypePref_Last18Months = 1 then GameType else null end as GameTypePref_Last18Months,
		case when GameTypePref_Last18Months = 1 and GameCodePref_Last18Months = 1 then GameCode else null end as GameCodePref_Last18Months,
		case when GameTypePref_Last12Months = 1 then GameType else null end as GameTypePref_Last12Months,
		case when GameTypePref_Last12Months = 1 and GameCodePref_Last12Months = 1 then GameCode else null end as GameCodePref_Last12Months,
		case when GameTypePref_Last9Months = 1 then GameType else null end as GameTypePref_Last9Months,
		case when GameTypePref_Last9Months = 1 and GameCodePref_Last9Months = 1 then GameCode else null end as GameCodePref_Last9Months,
		case when GameTypePref_Last6Months = 1 then GameType else null end as GameTypePref_Last6Months,
		case when GameTypePref_Last6Months = 1 and GameCodePref_Last6Months = 1 then GameCode else null end as GameCodePref_Last6Months,
		case when GameTypePref_Last4Months = 1 then GameType else null end as GameTypePref_Last4Months,
		case when GameTypePref_Last4Months = 1 and GameCodePref_Last4Months = 1 then GameCode else null end as GameCodePref_Last4Months,
		case when GameTypePref_Last3Months = 1 then GameType else null end as GameTypePref_Last3Months,
		case when GameTypePref_Last3Months = 1 and GameCodePref_Last3Months = 1 then GameCode else null end as GameCodePref_Last3Months,
		case when GameTypePref_Last2Months = 1 then GameType else null end as GameTypePref_Last2Months,
		case when GameTypePref_Last2Months = 1 and GameCodePref_Last2Months = 1 then GameCode else null end as GameCodePref_Last2Months,
		case when GameTypePref_LastMonth = 1 then GameType else null end as GameTypePref_LastMonth,
		case when GameTypePref_LastMonth = 1 and GameCodePref_LastMonth = 1 then GameCode else null end as GameCodePref_LastMonth,
		case when GameTypePref_3MonthLGD = 1 then GameType else null end as GameTypePref_3MonthLGD,
		case when GameTypePref_3MonthLGD = 1 and GameCodePref_3MonthLGD = 1 then GameCode else null end as GameCodePref_3MonthLGD
	FROM #GamingPreferencesBase_MM
	/*UNION  select cast(PlayerID as varchar(12)) + cast(CasinoCode as char(4)) as UniqueKey,
	...
	FROM #GamingPreferencesCORP_MM*/ ) a
    GROUP BY UniqueKey
 
	-- Create an index to improve processing
	CREATE CLUSTERED INDEX IDX_UniqueKey_2 ON #FinalPreferences_MM(UniqueKey)	    
	...
	INTO #MMGameTypePref_MM
	FROM [dbo].[IQ_Marketing_Metrics] b
	JOIN
		#SummedMarketingMetricsEXT_MM e
	ON b.UniqueKey=e.UniqueKey

	-- Create an index to improve processing
	CREATE CLUSTERED INDEX IDX_UniqueKey_3 ON #MMGameTypePref_MM(UniqueKey)
...
--added by Carl and Steve on 11/3/23
		,SlotActualWin_3MonthLGD
	...
	SELECT a.UniqueKey ,
		...
		,lastPlayDate_lastMonth
		,lastPlayDate_3MonthLGD
-- added by Carl and Steve on 11 /3/2023
		,SlotActualWin_3MonthLGD
		,TableNetActualWin_3MonthLGD
		,OtherNetActualWin_3MonthLGD
		,NetTheo_3MonthLGD
		,NetActualWin_3MonthLGD
	FROM #SummedMarketingMetricsEXT_MM a
	LEFT JOIN #UniqueDays_MM b ON a.UniqueKey = b.UniqueKey
	LEFT JOIN #MMGameTypePref_MM c ON a.UniqueKey = c.UniqueKey
	LEFT JOIN #FinalPreferences_MM d ON a.UniqueKey = d.UniqueKey
	LEFT JOIN ##PlayerType_MM p ON a.UniqueKey = p.UniqueKey
