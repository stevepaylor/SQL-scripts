...
FROM msdb_Feb06.dbo.sysjobs

where [name] LIKE 'Lawson%'
...
BEGIN
SELECT @jobname = name FROM msdb_Feb06.dbo.sysjobs WHERE job_id=@jobid
print @jobname

--inserting in sysjobs
--INSERT msdb.dbo.sysjobs
SELECT * FROM msdb_Feb06.dbo.sysjobs
WHERE job_id=@JobID

--inserting in sysjobsteps
--INSERT msdb.dbo.sysjobsteps
SELECT * FROM msdb_Feb06.dbo.sysjobsteps
WHERE job_id=@JobID
...
SELECT
    ...
    [message],run_status,run_date,run_time,run_duration,operator_id_emailed,
    operator_id_netsent,operator_id_paged,retries_attempted,[server]
FROM msdb_Feb06.dbo.sysjobhistory
WHERE job_id=@JobID
...
,s.[date_modified]
,s.[version_number]
FROM msdb_Feb06.dbo.sysschedules s, msdb_Feb06.dbo.sysjobschedules j
...
SELECT * FROM msdb_Feb06.dbo.sysjobschedules
WHERE job_id=@JobID
