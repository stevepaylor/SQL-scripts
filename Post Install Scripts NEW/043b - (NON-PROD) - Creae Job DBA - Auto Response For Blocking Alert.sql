...
SELECT @jobId = job_id FROM msdb.dbo.sysjobs WHERE (name = N'DBA - Auto Response For Blocking Alert (NON-PROD)')
...
/****** Object:  Job [DBA - Auto Response For Blocking Alert]    Script Date: 6/19/2019 1:56:21 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Viejas DB Maintenance]    Script Date: 6/19/2019 1:56:21 PM ******/
...
DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA - Auto Response For Blocking Alert (NON-PROD)', 
		@enabled=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [list blocking]    Script Date: 6/19/2019 1:56:22 PM ******/
...
BEGIN
   ...
   DECLARE @BODY_TEXT AS VARCHAR(200);
