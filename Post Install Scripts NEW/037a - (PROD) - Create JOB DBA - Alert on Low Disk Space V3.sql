...
/****** Object:  Job [DBA - Alert on Low Disk Space v3 (PROD)]    Script Date: 10/6/2025 4:45:13 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Viejas DB Maintenance]    Script Date: 10/6/2025 4:45:13 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA - Alert on Low Disk Space v3 (PROD)', 
		@enabled=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Check Space and Send Alerts]    Script Date: 10/6/2025 4:45:13 PM ******/
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			SET @msg_SubjectLine = ''!!! URGENT !!! -- '' + @@SERVERNAME + '' (PROD) '' + @msg_DriveLetter +''-Drive Is Critically Full - Freespace is '' + @msg_Pct_Free + ''% - '' +
			@msg_GB_Free + ''GB FREE'';
	...
	BEGIN
		SET @msg_SubjectLine = @@SERVERNAME + '' (PROD) '' + @msg_DriveLetter +''-Drive Is Under 20% Free -- Freespace is '' + @msg_Pct_Free + ''% - '' + @msg_GB_Free + ''GB FREE'';
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Every 30 minutes', 
		...
		@schedule_uid=N'12aafb84-eeea-4065-9f70-18cbdda2cb88'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
