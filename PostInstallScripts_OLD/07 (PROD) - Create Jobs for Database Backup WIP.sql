...
IF  EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'DBA - CommandLog Cleanup')
EXEC msdb.dbo.sp_delete_job @job_name=N'DBA - CommandLog Cleanup', @delete_unused_schedule=1
--USE [msdb]
--GO

IF  EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'DBA - Output File Cleanup')
EXEC msdb.dbo.sp_delete_job @job_name=N'DBA - Output File Cleanup', @delete_unused_schedule=1
--USE [msdb]
--GO
...
as
	...
	SET @BackupDirectoryFULL = 
	CASE  @FolderNo
	    WHEN 1 THEN '\\DBAMDB01BK\SQLBackup01' 
		WHEN 2 THEN '\\DBAMDB01BK\SQLBackup02' 
		WHEN 3 THEN '\\DBAMDB01BK\SQLBackup03' 
		WHEN 4 THEN '\\DBAMDB01BK\SQLBackup04' 
		WHEN 5 THEN '\\DBAMDB01BK\SQLBackup05' 
		WHEN 6 THEN '\\DBAMDB02BK\SQLBackup06' 
		WHEN 7 THEN '\\DBAMDB02BK\SQLBackup07' 
		WHEN 8 THEN '\\DBAMDB02BK\SQLBackup08' 
		ELSE '\\DBAMDB02BK\SQLBackup09'
	END;

	SET @BackupDirectoryDIFF = 
	CASE  @FolderNo
	    WHEN 1 THEN '\\DBAMDB01BK\SQLBackup01' 
		WHEN 2 THEN '\\DBAMDB01BK\SQLBackup02' 
		WHEN 3 THEN '\\DBAMDB01BK\SQLBackup03' 
	...
	BEGIN
		...
		EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName01, @step_name = @JobName01, @subsystem = 'CMDEXEC', @command = @JobCommand01,
...
@notify_level_page=0,
@notify_email_operator_name=N'ITappsDBA'
		EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName02, @step_name = @JobName02, @subsystem = 'CMDEXEC', @command = @JobCommand02, 
		--@output_file_name = @OutputFile02,
		@flags=@Flags, @retry_attempts = 1, @retry_interval = 10
...
@notify_level_page=0,
@notify_email_operator_name=N'ITappsDBA'
		EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName04, @step_name = @JobName04, @subsystem = 'CMDEXEC', @command = @JobCommand04, 
		--@output_file_name = @OutputFile04, 
		@flags=@Flags, @retry_attempts = 2, @retry_interval = 3
...
@notify_level_page=0,
@notify_email_operator_name=N'ITappsDBA'
		EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName10, @step_name = @JobName10, @subsystem = 'CMDEXEC', @command = @JobCommand10, 
...
@notify_level_page=0,
@notify_email_operator_name=N'ITappsDBA'
			EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName11, @step_name = @JobName11, @subsystem = 'CMDEXEC', @command = @JobCommand11, 
...
set	@BackupDirectoryINCR		= '\\DBAMBK01PR\SQLBackup03' -- Specify the backup root directory for the INCRs. Example: \\sequel\sql_incr   
...
/***********************************************************************************************************************************************
- Create Full Range of Schedules for the Full backup job
- After creation you must go in and enable the chosen schedule
***********************************************************************************************************************************************/
